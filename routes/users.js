...
const db = require("../data/database"); // establishes connection to database
...
router.get("/", async function (req, res) {
  const users = await db.getDb().collection("users").find().toArray(); // finds list of users in user document on database, then displays them as an array in the form of const users
...
router.post("/profiles", upload.single("image"), async function (req, res) {
  // using upload.single() because this post only accepts one upload. The "image" refers to the name of the object on the form-control input in new-user.ejs. The ordering of arguments here is important by the way.
  ...
  }); // inserts user info into the document (collection) named users
